...
function ajax(obj) {
	...
	xmlhttp.onreadystatechange = function () {    //这里扩展ajax回调事件
		if(xmlhttp.readyState !==4) return;//忽略未完成的请求
        if(timeout) {
        	if(obj.error){
        		obj.error();
        	}
	...
	if (type == 'POST' || type == 'OPTIONS') {
		xmlhttp.open(type, obj.url, obj.async || true);
		xmlhttp.setRequestHeader('Content-type', obj.contentType || 'application/x-www-form-urlencoded');
		if(obj.parseData){
			xmlhttp.send(_params(obj.data || null));
		}else{
			xmlhttp.send(obj.data || null);
		}
