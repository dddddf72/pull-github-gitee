    ...
    View.prototype.init = function(a, index) {
      debugger
        if (a.length > 0) {
            ...
            this.map = sort(a);
            this.focusId = typeof index == 'undefined' ? getFirst(this.map, this.a) : Number(index);//view.js 441
        } else {
      ...
      // debugger
        ...
        if (next) {
            ...
            if (this[action] && action !== 'ok') {
                //this.onblur();
                this[action].call(this.context, this.focusItem());
            } else {
