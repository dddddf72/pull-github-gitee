...
document.onsystemevent = function(e) {
    ...
    if(code == 5301){
        userId = 0 ;
        setGlobalVar('userId','');
        showMsg(tipUrl + "iPG/T-nsp/tip/a_collect.htm", "卡已拔出");
    }else if(code == 5372){
        var nev = {
        ...
        showMsg(tipUrl + "iPG/T-nsp/tip/a_collect.htm", "卡已插入");
    }
    // if(keyType == 1001) {
    // 	switch(code) {
    // 		case 40070://中山，卡插入
	...
	if(recJson.length != 0) {//推荐海报数据长度
		...
		if(recJson.length - 1 < keycode || keycode < 0) {//页面显示的推荐海报个数
            ...
            if(recJson[keycode].assetInfo.recordType=="pakg"){
                window.location.href =tipUrl+ "/iPG/T-nsp/dsjc/detail_dsjc.htm?userId=" + getUserId() + "&columnMapId=" + recJson[keycode].assetInfo.columnMapId+"&checkBookmark=Y&columnId="+recJson[keycode].assetInfo.columnId;
            }else{
                window.location.href = tipUrl+ "/iPG/T-nsp/jtyy/detail_jtyy.htm?userId=" + getUserId() + "&columnMapId=" + recJson[keycode].assetInfo.columnMapId+"&checkBookmark=Y&columnId="+recJson[keycode].assetInfo.columnId;
            }
		}
	}
}
...
function goToTop() {
    // saveUrlPath();saveTopUrlPath();
    // window.location.href = goUrl + "/iPG/T-nsp/top/vod_top.htm";
}
	...
	collect : function(columnId, assetId, assetName,custom) {
		if(checkUser()){
			...
			var VOD_saveVodFavorites = {
				...
				"callBack" : function(_dataJson) {
					if(_dataJson.code){
						showMsg(tipUrl + "iPG/T-nsp/tip/a_collect.htm","收藏已达到最大收藏记录数");
						return;
					}
					...
					if(bookmarkedId==0){
						...
						showMsg(tipUrl + "iPG/T-nsp/tip/a_collect.htm", msg);
					}else{
						var msg=_dataJson.message;;
						showMsg(tipUrl + "iPG/T-nsp/tip/a_collect.htm", "系统繁忙！");
					}
				}
			};
	...
	recmd : function(assetId,providerId,recommandPoint) {//推荐
		if(checkUser()){
			...
			var VOD_recommendAsset = {
				...
				"callBack" : function(_dataJson) {
					if(_dataJson.code){
						showMsg(tipUrl + "iPG/T-nsp/tip/a_collect.htm",_dataJson.message );
						return;
					}
...
IEPG.subText1 = function(_str, _subLength, _num,_width,_height) {
    var temp1 = _str.replace(/[^\x00-\xff]/g, "**");
    var temp2 = temp1.substring(0, _subLength);
    var x_length = temp2.split("\*").length - 1;
    var hanzi_num = x_length / 2;
...
function showMsg(url, msg) {
	...
	if(url == "") {
		url = tipUrl + "iPG/T-nsp/tip/a_busyInfo.htm";
		if(msg == "") {
			msg = "系统忙，请稍后再试！";
		}
	}
