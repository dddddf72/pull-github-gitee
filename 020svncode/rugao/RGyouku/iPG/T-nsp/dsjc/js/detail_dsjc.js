...
function getAssetDetail(_dataJson){
    ...
    //sumListLength = mediaJson.length;
...
function showMediaPage(pageJson) //显示剧集信息
{
	...
	curPage = pageInfoJson.curPage;
	if(countPage == 0)
	{
		curPage = 0;
	}
	document.getElementById(pageIdJson.pageInfo).innerHTML = curPage + "/" + countPage ;
...
function showMediaButton(curPage) //显示剧集按钮
{
	...
	var obj = document.getElementById("num_page_jump");
	if(obj !=null ){
		var str = "";

		//var countPage ;
		if(mediaJson.length % pageSize == 0){
			countPage = parseInt(mediaJson.length / pageSize);
		}else{
			countPage =parseInt(mediaJson.length / pageSize)+1;
		}

		for (var k = 0 ; k < countPage ; k++){
			if(k==countPage-1){
				var a = "\"numPrevPage focus\"";
				str += "<button type='button' onclick='turnMediaPage("+(k+1)+")' class='numPrevPage' style='left:"+(k*180)+"px' id='num_prevPage"+(k+1)+"' onFocus='setIdValue(this.id);changeObjClass(this,\"numPrevPage focus\")' onBlur='changeObjClass(this,\"numPrevPage\");'  value='"+((k * pageSize) + 1)+"-"+mediaJson.length+"集'>"+((k * pageSize) + 1)+"-"+mediaJson.length+"集</button>";

			}else{
				str += "<button type='button' onclick='turnMediaPage("+(k+1)+")' class='numPrevPage' style='left:"+(k*180)+"px' id='num_prevPage"+(k+1)+"' onFocus='setIdValue(this.id);changeObjClass(this,\"numPrevPage focus\")' onBlur='changeObjClass(this,\"numPrevPage\");' value='"+((k * pageSize) + 1)+"-"+((k+1) * pageSize)+"集'>"+((k * pageSize) + 1)+"-"+(k+1) * pageSize+"集</button>";
			}
	...
	obj.innerHTML = str;
	//保存焦点
	//document.getElementById("num_prevPage"+curPage).focus();
	}
...
//			var tempJson =  mediaJson[i];
			buttons[j].innerHTML = mediaJson[i].chapter;
			//buttons[j].value = j;
...
function checkBookmark(_dataJson) {
    ...
    if(bookmarkFlag == "true"){
		...
		var playJson = mediaJson[bookNumFocus];
		playJson.folderAssetId = zybData.parentAssetId;    //栏目ID
		...
		playJson.columnId = zybData.parentAssetId;//栏目ID
		playJson.columnMapId=zybData.assetId;     //媒资包ID
		
		//buyPlayType为1续播，为0新播
...
function onPlayMedia(index)
{
	...
	//检查书签获得续播时间点
	 var VOD_checkBookmark = { //folderAssetId用资源包assetId
        'data':'<CheckSavedProgram  folderAssetId="'+titleAssetId+'" portalId="'+portalId+'" client="'+cardId+'" account="'+userId+'" assetId="'+mediaJson[index].assetId+'" />',
...
function setGlobalToPlayNext(_index){
    ...
    setGlobalVar("sumListLength",mediaJson.length);
    setGlobalVar("chapter", mediaJson[_index].chapter);//保存当前播放章节数
    setGlobalVar("pakgId", zybData.assetId);//保存当前资源包的id，用于电视剧播放下一集
}
...
function moveRight(){
	...
	if(numId=="mediaBtn_10"){
	   document.getElementById("mediaBtn_11").focus();
	}else if(numId=="mediaBtn_20"){
	   if(curPage<countPage){
		    turnMediaPage(curPage+1);
			document.getElementById("mediaBtn_1").focus();
	   }
	}else if(numId==("num_prevPage"+(countPage-1))){
        document.getElementById("num_prevPage"+(countPage)).focus();
    }else if(numId==("num_prevPage"+(1))){
        if(document.getElementById("num_prevPage"+2))
        {
            document.getElementById("num_prevPage" + (2)).focus();
        }
    }else if(numId==("num_prevPage"+(countPage))){
        if(document.getElementById("mediaBtn_1"))
        {
            document.getElementById("mediaBtn_1").focus();
        }
    }
...
function moveLeft(){
	...
	if(numId=="mediaBtn_11"){
	   document.getElementById("mediaBtn_10").focus();
	}else if(numId=="mediaBtn_1"){
	   if(curPage>1){
		   turnMediaPage(curPage-1);
		   document.getElementById("mediaBtn_1").focus();
	   }
	}else if(numId==("num_prevPage"+(countPage))){
        document.getElementById("num_prevPage"+(countPage-1)).focus();
    }
}
function moveUp(){

	if(numId==("num_prevPage"+1)){
        document.getElementById("inputOKButton").focus();
    }
}
