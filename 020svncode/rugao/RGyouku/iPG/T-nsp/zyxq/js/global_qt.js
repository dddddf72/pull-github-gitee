...
var goUrl1 = location.href.split("/RGyouku")[0];
...
//var cardId = "8230110363107899";
//var cardId ="80183045662";
//var clientId ="4117671483";
/** userId 为用户ID */
...
function checkUser() {
	...
	if(userId == "" || userId == "0" || userId == undefined) {
		showMsg(tipUrl + "T-nsp/tip/a_collect.htm", "亲，您还没有开通点播业务，现在拨打电话96296， 我们会登门为您办理。");
		return false;
	}
	return true;
}
...
function grabEvent(_e) {
	...
	if(tipFlag) {
		...
		switch(keycode) {
			...
			case KEY.UP_N:
            case KEY.UP_RG:
                doUpPress(_e, keycode);
				break;
			case KEY.DOWN:
			case KEY.DOWN_N:
            case KEY.DOWN_RG:
                doDownPress(_e, keycode);
				break;
			case KEY.LEFT:
			case KEY.LEFT_N:
            case KEY.LEFT_RG:
                doLeftPress(_e, keycode);
				break;
			case KEY.RIGHT:
			case KEY.RIGHT_N:
            case KEY.RIGHT_RG:
                doRightPress(_e, keycode);
				break;
			case KEY.NEXT:
			case KEY.NEXT_N:
			...
			case KEY.ENTER:
			case KEY.ENTER_N:
                if (document.activeElement.type == "text")//输入框类型
                {
                    doInputOkPress(_e, keycode);
                }
				break;
			case KEY.PLAY:
			case KEY.PLAY1:
...
function goToSearch() {
	...
	// window.location.href =goUrl+ "/iPG/T-nsp/search/search.htm";
}
	...
	if(recJson.length != 0) {//推荐海报数据长度
		...
		if(recJson.length - 1 < keycode || keycode < 0) {//页面显示的推荐海报个数
            ...
            if(recJson[keycode].assetInfo.recordType=="pakg"){
                window.location.href =goUrl+ "/iPG/T-nsp/dsjc/detail_dsjc.htm?userId=" + getUserId() + "&columnMapId=" + recJson[keycode].assetInfo.columnMapId+"&checkBookmark=Y&columnId="+recJson[keycode].assetInfo.columnId;
            }else{
                window.location.href = goUrl+ "/iPG/T-nsp/jtyy/detail_jtyy.htm?userId=" + getUserId() + "&columnMapId=" + recJson[keycode].assetInfo.columnMapId+"&checkBookmark=Y&columnId="+recJson[keycode].assetInfo.columnId;
            }
		}
	}
}
...
function goToMyZone() {
    saveUrlPath();saveTopUrlPath();
    window.location.href = goUrl + "/iPG/T-nsp/myzone/PlayHistory.htm";
}
...
function goToPortal() {//清除路径
    ...
    setGloblaVar("cate_Id","");
    setGloblaVar("jtyygq_cate_Id","");
    setGloblaVar("jtyylist_cate_Id","");
    setGloblaVar("jtyysimple_cate_Id","");
	setGlobalVar("tzmyFlag", '');
	setGlobalVar("folderAssetIdT", '');
    setGlobalVar("isBack","");
			...
			var VOD_saveVodFavorites = {
				...
				"callBack" : function(_dataJson) {
					...
					if(bookmarkedId==0){
						...
						//window.document.body.style.background = "url(images/details_dsjc_bg2.png)";
			...
			var VOD_removeVodFavorites = {
				...
				"callBack" : function(_dataJson) {
					if(_dataJson.code == 0 ){
						...
						//删除成功
						//window.document.body.style.background = "url(images/details_dsjc_bg.png)";
	...
	if(imageList){
		for(var i = 0; i < imageList.length; i++){
			...
			if(distance < standard){
				standard = distance;
				picUrl = goUrl1+"/poster_root/"+imgUrl;
			}
		}
...
function goToDetail(dataJson)
{
    if(dataJson)
    {
        ...
        if(dataJson.isPackage=="1"){
            window.location.href =goUrl+ "/iPG/T-nsp/dsjc/detail_dsjc.htm?userId=" + getUserId() + "&titleAssetId=" + dataJson.assetId+"&checkBookmark=Y&folderAssetId="+dataJson.folderAssetId+"&serviceId="+dataJson.goodsId+"&providerId="+dataJson.providerId;
			
        }else{
            //window.location.href = goUrl+ "/iPG/T-nsp/jtyy/detail.htm?userId=" + getUserId() + "&titleAssetId=" + dataJson.assetId+"&checkBookmark=Y&folderAssetId="+dataJson.folderAssetId+"&serviceId="+dataJson.goodsId+"&providerId="+dataJson.providerId;
			window.location.href = goUrl+ "/iPG/T-nsp/jtyy/detail_jtyy.htm?userId=" + getUserId() + "&titleAssetId=" + dataJson.assetId+"&checkBookmark=Y&folderAssetId="+dataJson.folderAssetId+"&serviceId="+dataJson.goodsId+"&providerId="+dataJson.providerId;
        }
    }
}
...
function downShelfInfo(){
    ...
    var obj = document.getElementById(messInfoId);
	var url = "/iPG/T-nsp/yytc/a_downShelfInfo.htm";
	ajaxUrl(url, function(x)
	{
		obj.innerHTML = x.responseText;
		obj.style.display = "block";
