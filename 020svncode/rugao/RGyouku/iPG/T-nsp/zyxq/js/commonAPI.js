...
function checkBuy(_dataJson) {
    ...
    if(isBuy != "Y" && (Buy.chargeMode != 3)){
        var errorMsg = "亲，您还没有开通点播业务，是否试看";
...
IEPG.onPlayAction = function(dataArr, focusIndex) {
    var cardId =  CA.card.cardId || 0;
    var userId =  iPanel.getGlobalVar("userId") || 0;
	...
	if(dataArr) {
		...
		if(dataArr[focusIndex].isPackage == "1") {
			window.location.href = goUrl+ "/iPG/T-nsp/dsjc/detail.htm?providerId="+ dataArr[focusIndex].providerId + "&titleAssetId=" + dataArr[focusIndex].assetId +"&folderAssetId="+dataArr[focusIndex].folderAssetId;
		} else {
			var pmJson = {
...
IEPG.doPlay = function() {// 直接播放
	...
	if( Buy.tryFlag == "1"){//获取试看Token
		var VOD_getToken = {  
			"data":"<SelectionStart titleProviderId=\""+ Buy.providerId +"\" titleAssetId=\"" + Buy.resourceId + "\" folderAssetId=\""+tempColumnId+"\" serviceId=\"\" playPreview=\"Y\" portalId=\"" + portalId +"\" client=\"" + cardId +"\" account=\"" + userId +"\"/>",
...
function getTryToken(_dataJson){
	...
	setGlobalVar("new_vod_ctrl_rtsp", encodeURIComponent(_dataJson.rtsp));
    setGlobalVar("vod_ctrl_rtsp", encodeURIComponent(rtspUrl));
    setGlobalVar("new_vod_ctrl_rtsp", encodeURIComponent(rtspUrl));
...
function getPlaylist(_dataJson) {
	...
	setGlobalVar("assetId", Buy.resourceId);
	setGlobalVar("providerId", Buy.providerId);
	//保存当前资源包的上级id，用于电视剧播放下一集
	setGlobalVar("columnMapId", Buy.columnMapId);
	setGlobalVar("catalogId", Buy.columnMapId);//保存当前资源包的上级id，用于电视剧播放下一集
	setGlobalVar("folderAssetIdT", Buy.columnId);
	setGlobalVar("purchaseToken", purchaseToken);
	setGlobalVar("vod_play_type", Buy.buyPlayType);    //播放类型  0 正常, 1 续播
	setGlobalVar("serviceId", Buy.goodsId);
	...
	setGlobalVar("vod_ctrl_backurl", backUrl);
    delayTime = setTimeout(function() {
        //location.href = tipUrl + "iPG/T-nsp/BtvVodctrl/vodplay.htm";//新播控
