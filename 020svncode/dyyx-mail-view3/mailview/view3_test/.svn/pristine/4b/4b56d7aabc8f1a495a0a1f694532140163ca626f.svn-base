...
function operaterMethod() {
    ...
    switch (keyCode) {
        ...
        case MYKEY.LEFT:
            show();
            if (isPause) { return; }
            if (mp.getCurrentPlayTime() < 10) {
                mp.playByTime(1, 0, 0);
            } else {
                mp.playByTime(1, parseInt(mp.getCurrentPlayTime()) - 10, 1);
            }
        ...
        case MYKEY.RIGHT:
            show();
            if (isPause) { return; }
            if ((parseInt(mp.getCurrentPlayTime(), 10) + 10) < parseInt(mp.getMediaDuration())) {
                mp.playByTime(1, parseInt(mp.getCurrentPlayTime()) + 10, 1);
            } else {
