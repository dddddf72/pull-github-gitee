...
function Voices(voiceleft, voicesLen, defaultVoice) {
	...
	this.maxVoice = 100;				//音量100格为满格，每走一步+ 1 or - 1；
...
Voices.prototype.imgVoicePos = function() {		//音量位置对象
	return document.getElementById("imgPosition");
};
Voices.prototype.imgVoiceLen = function() {		//音量长度对象
	return document.getElementById("imgVoice");
};
Voices.prototype.txtVoice = function() {		//音量对象
	return document.getElementById("txtVoice");
};
...
Voices.prototype.setImgVoicePos = function(pos) {
	...
	cPos += pos;
	if(imgVoicePos) {
		cPos = this.voiceleft + cPos;
		imgVoicePos.style.left = cPos + "px";
	}
};
Voices.prototype.setImgVoiceLen = function(pos) {
	...
	if(imgVoiceLen)
		imgVoiceLen.width = cPos;
};
Voices.prototype.setTxtVoice = function(voice) {
	var txtVoice = this.txtVoice();
	if(voice > this.maxVoice)
		voice = this.maxVoice;
	if(voice < 0)
		voice = 0;
	if(txtVoice)
		txtVoice.innerHTML = voice;
};
Voices.prototype.clear = function() {
	this.currentVoice = 0;
	this.currentLen = 0;
};
Voices.prototype.UpDown = function(type) {
	this.currentVoice += type;
	if(this.currentVoice < 0 && type < 0) {
		this.currentVoice = 0;
		return 1;
	}//起点,无法再往前.
	if(this.currentVoice > this.maxVoice && type > 0) {
		this.currentVoice = this.maxVoice;
		return 2;
	}//终点，无法再往后
