...
function strArrayToString(strArray){
	SumaJS.debug(JSON.stringify(strArray)+"----------------");
	if(strArray == null){
		return null;
	}
	var i = 0;
...
function setApplication(param){
	...
	ajax.onreadystatechange = function(){
		if (ajax.readyState == 4 ){
			if(ajax.status == 200){
				if(SysSetting.getEnv('DataCollection'))SysSetting.deleteEnv('DataCollection');
			}
		}
	};		
	ajax.open("POST", dataCollectionUrl, true);
...
(function(){
    ...
    DataCollection.prototype.collectData = function(param){
        ...
        //SumaJS.debug("datacollect  :"+data);
        //if(param[0] == "04") {
        //    alert(data);
       // }
		sendDataByPost(data,false);
    };
    DataCollection.prototype.appCollData = function(param){
		if (isWork==false) {
			return;
		}
        var clientType = "01";
