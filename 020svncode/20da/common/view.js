/**
 * @author XboxYan
 * @document https://github.com/XboxYan/auto-keyboard
 */
var View = (function() {
    "use strict"
    //公共组件
                ...
                if (this.scrollY) {
                    ...
                    if (action == 'up') {
                        if (this.scrollAnimate) {
                            ...
                            if (this.parentNode.id == "viewParent") {
                    ...
                    if (action == 'down') {
                        if (this.scrollAnimate) {
                            ...
                            // if (this.parentNode.id == "nav" || this.parentNode.id == "viewParent") {
                            //     //首页左侧栏目板块，移动焦点至最后栏目，按下方向键会导致栏目消失问题
                            // } else {
    ...
    View.prototype.focus = function() {
        ...
        if (this.scrollX || this.scrollY) {
            ...
            if (this.scrollY) {
                if (this.scrollAnimate) {
                    ...
                    //this.parentNode.scrollTop = 100
                    // var top = offsetTop - (500 - 42) * .5;
                    var top = offsetTop - (500 - 35 - 42) * .4;

                    if (this.map.length * 80 < 500) {
                        top = 0
                    }
                    // console.log(top)
        ...
        if (this.cellFocus) this.cellFocus(this.focusItem());
    }
    ...
    View.prototype.blur = function(bool) {
        ...
        if (this.cellBlur) this.cellBlur(this.focusItem());
    }
    ...
    function sort(doms) {
        ...
        for (var j = 0; j < len; j++) {
            ...
            for (var index = 0; index < obj.length; index++) {
                var tag_right = (obj[index].cellX >= X + W * 0.5) && (obj[index].cellY >= Y && obj[index].cellY <= Y + H || obj[index].cellY <= Y && obj[index].cellY + obj[index].height >= Y)
                var tag_Bottom = (obj[index].cellY >= Y + H * 0.5) && (obj[index].cellX >= X && obj[index].cellX <= X + W || obj[index].cellX <= X && obj[index].cellX + obj[index].width >= X)
                var tag_Left = (obj[index].cellX + obj[index].width <= X + W * 0.5) && (obj[index].cellY >= Y && obj[index].cellY <= Y + H || obj[index].cellY <= Y && obj[index].cellY + obj[index].height >= Y)
                var tag_Top = (obj[index].cellY + obj[index].height <= Y + H * 0.5) && (obj[index].cellX >= X && obj[index].cellX <= X + W || obj[index].cellX <= X && obj[index].cellX + obj[index].width >= X)
                var cd_right = (obj[index].cellX + obj[index].width) > X + W
                var cd_Bottom = (obj[index].cellY + obj[index].height) > Y + H
                if (tag_right) {
                    Right_tmp.push(obj[index])
                }
                if (tag_Bottom) {
                    Bottom_tmp.push(obj[index])
                }
                if (tag_Left) {
                    Left_tmp.push(obj[index])
                }
                if (tag_Top) {
                    Top_tmp.push(obj[index])
                }
                if (cd_right) {
                    Right_cd.push(obj[index])
                }
                if (cd_Bottom) {
                    Bottom_cd.push(obj[index])
                }
            }
            var Right = Right_tmp.sort(function(a, b) { return (a.cellX - b.cellX) || (a.cellY + a.height * 0.5 + b.cellY + b.height * 0.5 - 2 * Y - H) * (a.cellY + a.height * 0.5 - b.cellY - b.height * 0.5) || (a.cellY - b.cellY) })[0];
            var Bottom = Bottom_tmp.sort(function(a, b) { return (a.cellY - b.cellY) || (a.cellX + a.width * 0.5 + b.cellX + b.width * 0.5 - 2 * X - W) * (a.cellX + a.width * 0.5 - b.cellX - b.width * 0.5) || (a.cellX - b.cellX) })[0];
            var Left = Left_tmp.sort(function(a, b) { return (b.cellX + b.width - a.cellX - a.width) || (a.cellY + a.height * 0.5 + b.cellY + b.height * 0.5 - 2 * Y - H) * (a.cellY + a.height * 0.5 - b.cellY - b.height * 0.5) || (a.cellY - b.cellY) })[0];
            var Top = Top_tmp.sort(function(a, b) { return (b.cellY + b.height - a.cellY - a.height) || (a.cellX + a.width * 0.5 + b.cellX + b.width * 0.5 - 2 * X - W) * (a.cellX + a.width * 0.5 - b.cellX - b.width * 0.5) || (a.cellX - b.cellX) })[0];
            // if (!Right) Right = Right_cd.sort(function(a, b) { return (Math.pow((X + W - a.cellX), 2) + Math.pow((Y + H / 2 - a.cellY), 2)) - ((Math.pow((X + W - b.cellX), 2) + Math.pow((Y + H / 2 - b.cellY), 2))) })[0];
            if (!Bottom) Bottom = Bottom_cd.sort(function(a, b) { return (Math.pow((X + W / 2 - (a.cellX + a.width / 2)), 2) + Math.pow((Y + H - a.cellY), 2)) - ((Math.pow((X + W / 2 - (b.cellX + b.width / 2)), 2) + Math.pow((Y + H - b.cellY), 2))) })[0];
    ...
    function keyCode(event) {
        ...
        switch (e) {
            ...
            case 13:
                preventDefault(event);
                action = 'ok';
                // if (hasClass($("checkout"), 'show') && hasClass($("checkoutConTxt"), 'show')) {
                //   doConfirmClose();
                // }
                break;
            case 8:
